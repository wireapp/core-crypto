...
impl CentralContext {
    pub(crate) async fn send_and_merge_commit(
        ...
        match self.send_commit(commit).await {
            Ok(()) => conversation.commit_accepted(&self.mls_provider().await?).await,
            Err(CryptoError::MessageRejected { reason }) => {
                conversation.clear_pending_commit(&self.mls_provider().await?).await?;
                Err(CryptoError::MessageRejected { reason })
            }
            Err(e) => Err(e),
        }
    ...
    pub(crate) async fn send_commit(&self, commit: MlsCommitBundle) -> CryptoResult<()> {
