...
use crate::{
    ...
    entities::{
        ...
        StoredCredential, StringEntityId, count_helper, count_helper_tx, delete_helper, get_helper, load_all_helper,
    },
    traits::{
...
#[async_trait]
impl BorrowPrimaryKey for StoredCredential {
        ...
        Q: KeyType,
    {
        ...
        stmt.query_row([key.bytes().as_ref()], Self::from_row)
            .optional()
            .map_err(Into::into)
    }
}
...
#[async_trait]
impl<'a> EntityDeleteBorrowed<'a> for StoredCredential {
    ...
    where
        ...
        Q: KeyType,
    {
        delete_helper::<Self>(tx, "public_key", id.bytes().as_ref()).await
    }
}
