    ...
    fn impl_primary_key(&self) -> TokenStream {
        ...
        quote! {
            ...
            impl crate::traits::BorrowPrimaryKey for #struct_name {
                type BorrowedPrimaryKey<'a> = &'a #borrowed_primary_key;
    ...
    fn impl_entity_get_borrowed(&self) -> TokenStream {
        ...
        quote! {
            impl crate::traits::EntityGetBorrowed for #struct_name {
                fn get_borrowed(conn: &rusqlite::Connection, key: Self::BorrowedPrimaryKey<'_>)
                    -> crate::CryptoKeystoreResult<Option<Self>>
                {
                    // Bind the borrowed key directly, exactly as the insert generated by
                    // `store_expression` binds the id field: `&str` binds as `TEXT` and `&[u8]` as
                    // `BLOB`, so the lookup and the insert cannot disagree about storage class.
        ...
        quote! {
            impl crate::traits::EntityDeleteBorrowed for #struct_name {
                fn delete_borrowed(
                    ...
                    id: <Self as crate::traits::BorrowPrimaryKey>::BorrowedPrimaryKey<'_>,
                ) -> crate::CryptoKeystoreResult<bool>
                {
                    // Bind the borrowed key directly, for the same reason as in `get_borrowed`.
                    crate::entities::helpers::delete_helper::<Self>(tx, #id_column_name, id)
                }
            }
        }
    }
