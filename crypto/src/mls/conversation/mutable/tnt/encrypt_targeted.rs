...
use super::{PskId, TargetedMessage, TargetedMessageContext, TntMessage, TntMessageTBS};
use crate::{
    ClientIdRef, CryptoProvider, OpenMlsError, RecursiveError,
    mls::conversation::{ConversationMut, Error, MlsGroupState, Result},
};
...
impl ConversationMut {
    ...
    pub async fn encrypt_targeted(
        ...
        let targeted = self
            .mutate_group_and_sender_nonce(async |_, group_state, _| {
    ...
    fn create_targeted_message(
        ...
        let nonce = group_state.obtain_sender_nonce()?;
