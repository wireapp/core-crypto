...
async fn dump_legacy_fixture() {
    ...
    db.save(ConsumerData {
        content: b"consumer data kept across the import".to_vec(),
    })
    .await
    .unwrap();
    db.save(E2eiAcmeCA {
        content: hex::decode(TRUST_ANCHOR_DER_HEX).unwrap(),
    })
    .await
    .unwrap();
    db.save(E2eiCrl {
    ...
    .await
    .unwrap();
    db.save(ProteusIdentity {
        sk: vec![0x51; 64],
        pk: vec![0x5b; 32],
    })
    .await
    .unwrap();
    db.save(ProteusPrekey::from_raw(42, b"a proteus prekey".to_vec()))
        .await
        .unwrap();
