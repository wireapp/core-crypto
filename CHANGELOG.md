...
- ⚠️ = tier 2 support. Things compile but _might_ not work as expected. Basically works but with papercuts
...
- [**breaking**] Remove all the final\_\* methods returning a TLS encoded CommitBundle
...
- All commit methods now return a `CommitBundle` struct containing
    _the commit message
    _ an optional `Welcome` if there were pending add proposals \* a `PublicGroupState` to upload to the Delivery Service
