...
use crate::{
    ...
    traits::{
        BorrowPrimaryKey, DecryptData as _, Decryptable, Decrypting, EncryptData as _, Encrypting, Entity, EntityBase,
        EntityGetBorrowed, KeyType, PrimaryKey, UniqueEntity,
    },
};
#[cfg(not(target_family = "wasm"))]
use crate::{
    connection::TransactionWrapper,
    traits::{EntityDatabaseMutation, EntityDeleteBorrowed},
};
...
static NEW_DUMMY_STORE_IDS: LazyLock<RwLock<HashSet<Vec<u8>>>> = LazyLock::new(|| {
    ...
    for idx in 0_u8..42 {
        hasher.update("Entity for NewDummyStoreValue: ".as_bytes());
...
impl EntityBase for NewDummyStoreValue {
    type ConnectionType = crate::connection::KeystoreDatabaseConnection;
    type AutoGeneratedFields = ();
...
#[cfg_attr(target_family = "wasm", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_family = "wasm"), async_trait::async_trait)]
impl Entity for NewDummyStoreValue {
...
impl<'a> EntityDatabaseMutation<'a> for NewDummyStoreValue {
    ...
    async fn save(&'a self, _tx: &Self::Transaction) -> CryptoKeystoreResult<()> {
