...
impl EntityBase for MlsSignatureKeyPair {
    ...
    async fn find_all(
        ...
        let query: String = format!(
            "SELECT rowid, signature_scheme, unixepoch(created_at) FROM mls_signature_keypairs {}",
            params.to_sql()
        );
        ...
        let entities = rows.try_fold(Vec::new(), |mut acc, rowid_result| {
            ...
            let (rowid, signature_scheme, created_at) = rowid_result?;
    ...
    async fn find_one(
        ...
        let maybe_rowid = transaction
            .query_row(
                "SELECT rowid, signature_scheme, unixepoch(created_at) FROM mls_signature_keypairs WHERE pk = ?",
                [id.as_slice()],
                |r| Ok((r.get::<_, i64>(0)?, r.get(1)?, r.get(2)?)),
            )
            .optional()?;

        if let Some((rowid, signature_scheme, created_at)) = maybe_rowid {
            let mut blob = transaction.blob_open(
                rusqlite::DatabaseName::Main,
                "mls_signature_keypairs",
                "pk",
    ...
    async fn insert(&self, conn: &mut Self::ConnectionType) -> CryptoKeystoreResult<Self::AutoGeneratedFields> {
        ...
        let sql = "INSERT INTO mls_signature_keypairs (signature_scheme, pk, keypair, credential_id) VALUES (?, ?, ?, ?) RETURNING unixepoch(created_at)";
...
impl MlsSignatureKeyPairExt for MlsSignatureKeyPair {
    async fn keypair_for_signature_scheme(
        ...
        let maybe_found = transaction
            .query_row(
                "SELECT rowid, unixepoch(created_at) FROM mls_signature_keypairs WHERE signature_scheme = ? AND credential_id = ?",
                [signature_scheme.to_sql()?, credential_id.to_sql()?],
                |r| Ok((r.get::<_, i64>(0)?, r.get::<_, u64>(1)?)),
            )
            .optional()?;

        if let Some((rowid, created_at)) = maybe_found {
            let mut blob = transaction.blob_open(
                rusqlite::DatabaseName::Main,
                "mls_signature_keypairs",
                "pk",
