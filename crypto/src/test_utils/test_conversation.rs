...
impl<'a> TestConversation<'a> {
    ...
    pub async fn are_members(&self, members_to_check: impl IntoIterator<Item = &'a SessionContext>) -> bool {
        ...
        let ids_to_check =
            futures_util::future::join_all(members_to_check.into_iter().map(|member| member.get_client_id())).await;

        ids_to_check.into_iter().all(|id| member_ids.contains(&id))
    }
    ...
    async fn try_talk(&self, member: &SessionContext, other_member: &SessionContext) -> crate::test_utils::Result<()> {
        let mut member_guard = self.guard_of(member).await;
        let mut other_guard = self.guard_of(other_member).await;
