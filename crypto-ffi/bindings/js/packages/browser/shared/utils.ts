...
export async function teardownBrowser() {
    // `setupBrowser` may have thrown partway through, leaving either of these unset. Tear down
    // whatever was created: a server left running keeps the event loop alive, so the test process
    // hangs forever instead of reporting the setup failure.
    await browser?.close();
    await server?.stop();
    browser = undefined;
    server = undefined;
