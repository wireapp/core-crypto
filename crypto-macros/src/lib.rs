...
///
/// All fields in an entity using this derive macro must have one of these types:
///
/// - `Vec<u8>`
/// - `String`
/// - `Option<Vec<u8>>`
...
/// - `bool`
///
/// There is one exception: the identity field can only be a `Vec<u8>` or `String`; none of the
/// other types, including the optional variant, are permitted in that context.
///
...
/// - `no_upsert`: Optional flag. By default the generated `save` upserts via `ON CONFLICT (id) DO UPDATE`. When this
///   flag is present it emits a plain `INSERT` instead, and a uniqueness violation is mapped to
...
///
/// ```sql
/// INSERT INTO entities (id, foo, bar) VALUES (?, ?, ?) ON CONFLICT (id) DO UPDATE SET foo = excluded.foo, bar = excluded.bar
/// ```
///
/// The `ON CONFLICT` clause is omitted when the `no_upsert` flag is set; see the attribute documentation above.
///
