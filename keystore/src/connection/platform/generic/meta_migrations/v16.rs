...
#[async_trait::async_trait]
impl<'a> EntityTransactionExt<'a> for StoredSignatureKeypair {
    type Transaction = TransactionWrapper<'a>;

    async fn count(transaction: &TransactionWrapper<'a>) -> CryptoKeystoreResult<u32> {
        todo!()
    }

    async fn save(&self, transaction: &TransactionWrapper<'a>) -> CryptoKeystoreResult<()> {
    ...
    async fn delete(transaction: &TransactionWrapper<'a>, id: &Self::PrimaryKey) -> CryptoKeystoreResult<bool> {
