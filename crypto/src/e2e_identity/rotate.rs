...
impl CentralContext {
    ...
    /// * Rotate credentials for each conversation in [Self::e2ei_rotate]
    /// * Generate new key packages with [Client::generate_new_keypackages]
    /// * Replace them in the backend
    /// * Delete them using [Self::retain_only_key_packages_with_matching_credential]
    ...
    /// saved credential with the provided signature scheme and credential type.
    pub async fn retain_only_key_packages_matching_most_recent_credentials(
        &self,
        ...
        let cb = client
            .find_most_recent_credential_bundle(signature_scheme, credential_type)
            .await
            .map_err(|_| E2eIdentityError::MissingExistingClient(credential_type))?;
            ...
            run_test_with_client_ids(
                ...
                move |[mut alice_central, mut bob_central, mut charlie_central]| {
                    Box::pin(async move {
                        ...
                        alice_central
                            .context
                            .retain_only_key_packages_matching_most_recent_credentials(
                                case.signature_scheme(),
                                case.credential_type,
                            )
                            .await
