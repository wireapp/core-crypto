...
$(STAMPS)/docs-ts: $(WASM_FILE) $(STAMPS)/bun-deps
...
WASM_FILE := crypto-ffi/bindings/js/src/autogenerated/core-crypto-ffi_bg.wasm
$(WASM_FILE): $(RUST_SOURCES)
	cd crypto-ffi && \
	$(WASM_PACK) build \
	  --locked \
	  --no-pack \
