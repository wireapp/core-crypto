    ...
    mod update {
        ...
        pub async fn renewable_when_created_by_self(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                let propposal_guard = conversation.update_proposal_guarded().await;
        ...
        pub async fn not_renewable_by_ref(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                let propposal_guard = conversation.acting_as(&bob).await.update_proposal_guarded().await;
                assert!(alice.pending_proposals(&id).await.is_empty());
    ...
    mod add {
        ...
        pub async fn not_renewable_by_ref(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                let proposal_guard = conversation
                    .acting_as(&bob)
                    .await
                    .invite_proposal_guarded(&debbie)
                    .await
                    .notify_member(&alice)
                    .await;
                assert_eq!(alice.pending_proposals(&id).await.len(), 1);
        ...
        pub async fn renewable_when_valid_commit_doesnt_adds_same(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                assert!(alice.pending_proposals(&id).await.is_empty());
                let propposal_guard = conversation.invite_proposal_guarded(&charlie).await;
                assert_eq!(alice.pending_proposals(&id).await.len(), 1);
    ...
    mod remove {
        ...
        pub async fn not_renewable_by_ref(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                let proposal_guard = conversation
                    .acting_as(&bob)
                    .await
                    .remove_proposal_guarded(&charlie)
                    .await
                    .notify_member(&alice)
                    .await;
                assert_eq!(alice.pending_proposals(&id).await.len(), 1);
        ...
        pub async fn renewable_when_valid_commit_doesnt_remove_same(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                assert!(alice.pending_proposals(&id).await.is_empty());
                let propposal_guard = conversation.remove_proposal_guarded(&charlie).await;
                assert_eq!(alice.pending_proposals(&id).await.len(), 1);
