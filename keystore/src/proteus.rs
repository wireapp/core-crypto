use crate::{
    ...
    entities::{Entity as _, ProteusPrekey},
};
...
impl proteus_traits::PreKeyStore for Connection {
    ...
    async fn prekey(
        ...
        Ok(self
            .find::<ProteusPrekey>(&ProteusPrekey::to_entity_id(id.to_le_bytes())?)
            .await?
            .map(|db_prekey| db_prekey.prekey.clone()))
    }

    async fn remove(&mut self, id: proteus_traits::RawPreKeyId) -> Result<(), Self::Error> {
        Connection::remove::<ProteusPrekey>(self, &ProteusPrekey::to_entity_id(id.to_le_bytes())?).await?;
