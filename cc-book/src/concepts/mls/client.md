...
In MLS terminology a _client_ is a single device acting on behalf of a user — each phone, desktop, or web session that
...
> [!NOTE]
> The Rust library internally calls this type `Session` to avoid ambiguity — "client" means something different at the
> Wire application layer (where it refers to a registered device in the Wire backend sense) and at the MLS protocol
> layer. The name `Session` does not appear in platform bindings; from Swift, Kotlin, and TypeScript, the type is always
> `CoreCrypto`.
...
Most MLS work happens inside a transaction (see [Transactions and the TransactionContext](transactions.md)), but a
