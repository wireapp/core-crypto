...
mod tests {
    ...
    async fn should_buffer_and_reapply_messages_after_commit_merged_for_receivers(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let external_proposal = proposal_guard.message();
            let conversation = proposal_guard.finish();
            let proposal_guard = conversation.acting_as(&bob).await.update_proposal_guarded().await;
            let proposal = proposal_guard.message();
            let commit_guard = proposal_guard
                .finish()
                .acting_as(&bob)
                .await
                .invite_guarded([&debbie])
                .await;
            let commit = commit_guard.message();
            let conversation = commit_guard.process_member_changes().await.finish();
    ...
    async fn wpb_15810(mut case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let proposal_guard = conversation
                .external_remove_proposal_guarded(&external_0, &member_114)
                .await;
            let proposal_remove_114_1 = proposal_guard.message();
            let conversation = proposal_guard.finish();

            // now bump the epoch in external_0: the new member has joined
            let (commit_guard, mut pending_conversation) =
                conversation.unmerged_external_join_guarded(&new_member).await;
            ...
            let proposal_guard = conversation
                .external_remove_proposal_guarded(&external_0, &member_115)
                .await;
            ...
            let (commit_guard, result) = commit_guard.notify_member_failible(&new_member).await;
            ...
            assert!(
                conversation
                    .is_functional_with([&observer, &new_member, &member_27])
                    .await
            )
        })
        .await
