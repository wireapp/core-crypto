    ...
    pub(crate) mod all {
        ...
        async fn enrollment_should_rotate_all(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                for commit in result.commits.into_iter() {
                    let (commit, decrypted) = commit.notify_member_failible(&bob).await;
                ...
                let conversation = case
                    ...
                    .await
                    .invite_with_credential_type(MlsCredentialType::X509, [&alice])
                    .await;
        ...
        async fn rotate_should_roundtrip(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                alice
                    ...
                    .await
                    .unwrap();
                let conversation = conversation.e2ei_rotate_and_verify_sender(None).await;
                ...
                conversation
                    .acting_as(&bob)
                    .await
                    .e2ei_rotate_and_verify_sender(None)
                    .await;
    ...
    mod one {
        ...
        pub async fn should_rotate_one_conversations_credential(case: TestContext) {
            if case.is_x509() {
                ...
                Box::pin(async move {
                    ...
                    let _conversation = conversation.e2ei_rotate_and_verify_sender(Some(&cb)).await;
        ...
        pub async fn rotate_should_be_renewable_when_commit_denied(case: TestContext) {
            ...
            Box::pin(async move {
                ...
                // Alice issues an Update commit to replace her current identity
                let conversation = conversation.unmerged_e2ei_rotate(&cb).await.finish();
        ...
        pub async fn rotate_should_replace_existing_basic_credentials(case: TestContext) {
            if case.is_x509() {
                ...
                Box::pin(async move {
                    ...
                    let _conversation = conversation.e2ei_rotate_and_verify_sender(None).await;
