...
import type { ClientIdInterface, DatabaseKeyInterface, EpochObserver, HistoryObserver } from "./index.web";
...
export class CoreCrypto {
    ...
    close() {
        if (CoreCryptoFfi.instanceOf(this.#cc)) {
            ...
            throw new Error("CoreCrypto hasn't been initialized properly. Context: closing")
        }
    }
    ...
    proteusLastResortPrekeyId(): number {
        return this.#cc.proteusLastResortPrekeyId();
    }
     ...
     * @returns Hex-encoded public key string
     **/
    proteusFingerprintPrekeybundle(prekey: ArrayBuffer): string {
        try {
            return this.#cc.proteusFingerprintPrekeybundle(prekey);
        } catch (e) {
