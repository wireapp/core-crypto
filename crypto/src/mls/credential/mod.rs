...
pub use self::credential_ref::CredentialRef;
pub use self::credential_ref::FindFilters;
pub use self::credential_ref::FindFiltersBuilder;
...
mod tests {
    ...
    async fn should_fail_when_signature_key_doesnt_match_certificate_public_key(case: TestContext) {
        ...
        let new_pki_kp = PkiKeypair::rand(case.signature_scheme(), CRYPTO.as_ref()).unwrap();
