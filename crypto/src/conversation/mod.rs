...
impl MlsCentral {
    ...
    pub(crate) fn get_conversation_mut<'a, T>(
        groups: &'a mut HashMap<ConversationId, MlsConversation>,
        id: &ConversationId,
    ) -> CryptoResult<T>
    where
        T: TryFrom<&'a mut MlsConversation>,
        CryptoError: From<<T>::Error>,
    {
        groups
            .get_mut(id)
            .ok_or_else(|| CryptoError::ConversationNotFound(id.clone()))
            .and_then(|c| Ok(c.try_into()?))
    }
}
...
pub mod state_tests_utils {
    ...
    pub async fn conv_pending_proposal_and_no_pending_commit(central: &mut MlsCentral, id: &Vec<u8>) {
        central
            ...
            .await
            .unwrap();
        let _ = central.new_proposal(id.clone(), MlsProposal::Update).await.unwrap();
        let conv = central.get_conversation(id).unwrap();
    ...
    pub mod create {
        ...
        pub async fn create_1_1_conversation_should_succeed(credential: CredentialSupplier) {
            ...
            let alice_can_send_message = alice_group
                ...
                .await;
            assert!(alice_can_send_message.is_ok());
            let bob_can_send_message = bob_group.as_can_encrypt().encrypt_message(b"me", &bob_backend).await;
