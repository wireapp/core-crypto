...
pub(crate) trait UniqueEntityExt<'a>: UniqueEntity + EntityDatabaseMutation<'a> {
...
pub(crate) trait UniqueEntityImplementationHelper {
    fn new(content: Vec<u8>) -> Self;
    fn content(&self) -> &[u8];
}
...
where
    T: EntityBase<ConnectionType = KeystoreDatabaseConnection>
...
where
    ...
    <T as PrimaryKey>::PrimaryKey: KeyType,
{
