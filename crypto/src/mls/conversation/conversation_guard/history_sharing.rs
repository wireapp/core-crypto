...
mod tests {
    ...
    pub async fn enable_disable_history_sharing(case: TestContext) {
        let [alice] = case.sessions().await;
        Box::pin(async move {
            let test_conv = case.create_conversation([&alice]).await;
            let mut guard = test_conv.guard().await;
            ...
            guard.enable_history_sharing().await.unwrap();
            assert!(guard.is_history_sharing_enabled().await);
            assert_eq!(test_conv.members_counted_by(&alice).await, 2);

            guard.disable_history_sharing().await.unwrap();
            assert!(!guard.is_history_sharing_enabled().await);
            assert_eq!(test_conv.member_count().await, 1);
