...
#[derive(Clone, Copy, TlsSize, TlsDeserialize, TlsSerialize, derive_more::From, PartialEq, Eq)]
...
impl TntWireFormat {
    ...
    /// The current rx counter value of the given message type, which depends on the given sender, conversation, and
    /// epoch.
    ///
    ...
    async fn current_rx_counter(
        &self,
        database: &impl FetchFromDatabase,
        primary_key: MessageRxCounterPkRef<'_>,
    ) -> Result<u32> {
        let count = match *self {
            // Both transient message types share an rx counter table because they are sent over the
            // same channel.
            Self::TRANSIENT_MESSAGE | Self::TRANSIENT_TARGETED_MESSAGE => database
                .get_borrowed::<TransientMessageRxCounter>(primary_key)
                .await
                .map_err(KeystoreError::wrap("getting TransientMessageRxCounter"))?
                .map(|counter| counter.count)
                .unwrap_or_default(),
            // Targeted messages have their own rx counter type.
