...
class InteropActionHandler(val coreCrypto: CoreCryptoClient) {
    ...
    suspend fun handleAction(action: InteropAction): Result<String> {
        return when(action) {
            ...
            is InteropAction.MLS.GetKeyPackage -> {
                coreCrypto.transaction { context ->
                    context.clientKeypackages(
                        ciphersuiteFromU16(action.ciphersuite.toUShort()), CredentialType.BASIC,
                        1u
                    )
                }.let {
                    Result.success(Base64.Default.encode(it.get(0).copyBytes()))
                }
            }

            is InteropAction.MLS.ProcessWelcome -> {
                coreCrypto.transaction { context ->
                    context.processWelcomeMessage(Welcome(action.welcome), CustomConfiguration(null, null))
                }.let {
