...
async fn run_mls_test(chrome_driver_addr: &std::net::SocketAddr) -> Result<()> {
    ...
    let configuration =
        MlsCentralConfiguration::try_new("whatever".into(), "test".into(), MLS_MAIN_CLIENTID.into(), ciphersuites)?;
...
async fn run_proteus_test(chrome_driver_addr: &std::net::SocketAddr) -> Result<()> {
    ...
    let configuration = MlsCentralConfiguration::try_new(
        "whatever".into(),
        "test".into(),
        MLS_MAIN_CLIENTID.into(),
        vec![MlsCiphersuite::default()],
    )?;
    let mut master_client: CoreCrypto = MlsCentral::try_new_in_memory(configuration, None).await?.into();
    ...
    for i in 0..ROUNDTRIP_MSG_AMOUNT {
        ...
        let mut messages_to_decrypt = master_client.proteus_encrypt_batched(&master_sessions, &message)?;
