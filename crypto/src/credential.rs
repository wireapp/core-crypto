...
pub mod tests {
    ...
    async fn basic_clients_can_send_messages() {
        ...
        alice_and_bob_can_talk(alice_cred, bob_cred).await.unwrap();
    }
    ...
    async fn should_fail_when_certificate_chain_is_empty() {
        ...
        let bob_cred = CertificateBundle::rnd_certificate_bundle();
        assert!(matches!(
            alice_and_bob_can_talk(alice_cred, bob_cred).await.unwrap_err(),
    ...
    async fn alice_and_bob_can_talk(alice_cred: CredentialSupplier, bob_cred: CredentialSupplier) -> CryptoResult<()> {
        ...
        alice_central.can_talk_to(&id, &mut bob_central).await
    }
}
