...
Each method returns an `MlsTransportResponse` indicating the outcome:

`Success`::
  The delivery service accepted the message.

`Retry`::
  The delivery service rejected the message because the client is behind — it has not yet processed all pending incoming messages.
  CoreCrypto will not retry automatically; the caller should fetch and process pending messages, then re-attempt the operation that triggered the callback.

`Abort { reason }`::
  The delivery service rejected the message and there is no recovery path.
  CoreCrypto surfaces this as an `MlsError::MessageRejected` with the reason string attached.
