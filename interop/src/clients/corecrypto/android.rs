...
impl SimulatorDriver {
    ...
    fn launch_application(
        ...
        let pid = String::from_utf8(pidof.stdout).expect("pidof output is not valid utf8");
...
impl CoreCryptoAndroidClient {
    pub(crate) async fn new() -> Result<Self> {
        ...
        let device = std::env::var("INTEROP_EMULATOR_DEVICE").unwrap_or("device".into());
...
impl EmulatedMlsClient for CoreCryptoAndroidClient {
    ...
    async fn kick_client(&self, conversation_id: &[u8], client_id: &[u8]) -> Result<()> {
        ...
        self.driver
            .execute(format!("--es action remove-client --es cid {} --es client {}", cid_base64, client_id_base64))
            .await?;
    ...
    async fn process_welcome(&self, welcome: &[u8]) -> Result<Vec<u8>> {
        ...
        let conversation_id_base64 = self
            .driver
            .execute(format!(
                "--es action process-welcome --es welcome {}",
                welcome_base64
            ))
            .await?;
    ...
    async fn encrypt_message(&self, conversation_id: &[u8], message: &[u8]) -> Result<Vec<u8>> {
        ...
        let encrypted_message_base64 = self
            .driver
            .execute(format!("--es action encrypt-message --es cid {} --es message {}", cid_base64, message_base64))
            .await?;
    ...
    async fn decrypt_message(&self, conversation_id: &[u8], message: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        let result = self
            .driver
            .execute(format!("--es action decrypt-message --es cid {} --es message {}", cid_base64, message_base64))
            .await?;
...
impl crate::clients::EmulatedProteusClient for CoreCryptoAndroidClient {
    ...
    async fn get_prekey(&self) -> Result<Vec<u8>> {
        ...
        let prekey_base64 = self.driver.execute(format!("--es action get-prekey --es id {}", prekey_last_id)).await?;
