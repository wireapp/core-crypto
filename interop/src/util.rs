...
pub(crate) async fn cp_wasm_files(wasm_deploy_path: PathBuf) -> Result<()> {
    ...
    std::fs::copy(
        cwd.join("crypto-ffi/bindings/js/out/browser/corecrypto.js"),
        wasm_deploy_path.join("corecrypto.js"),
    )?;

    std::fs::copy(
        cwd.join("crypto-ffi/bindings/js/out/browser/corecrypto.d.ts"),
    ...
    std::fs::copy(
        cwd.join("crypto-ffi/bindings/js/out/browser/autogenerated/wasm-bindgen/index_bg.wasm"),
