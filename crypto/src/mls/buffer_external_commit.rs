...
impl CentralContext {
    ...
    async fn incoming_message_is_own_join_commit(&self, id: &ConversationId, message: &[u8]) -> CryptoResult<bool> {
        let backend = self.mls_provider().await?;
        // Instantiate the pending group
        let (group, _cfg) = backend.key_store().mls_pending_groups_load(id).await?;
        ...
        mls_group.merge_pending_commit(&backend).await.map_err(MlsError::from)?;
        let message_in = MlsMessageIn::tls_deserialize(&mut message.as_ref()).map_err(MlsError::from)?;
        ...
        let group_ct = mls_group.compute_confirmation_tag(&backend).map_err(MlsError::from)?;
        Ok(*msg_ct == group_ct)
    }
    ...
    async fn merge_pending_group_and_build_decrypted_message_instance(
        ...
        let own_leaf = conversation.group.own_leaf().ok_or(CryptoError::InternalMlsError)?;
        ...
        let identity = own_leaf_credential_with_key.extract_identity(conversation.ciphersuite(), None)?;

        let crl_new_distribution_points =
            get_new_crl_distribution_points(&backend, extract_crl_uris_from_group(&conversation.group)?).await?;
