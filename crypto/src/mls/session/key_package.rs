...
mod tests {
    ...
    async fn requesting_x509_key_packages_after_basic(case: TestContext) {
        ...
        assert!(session.transaction.e2ei_is_enabled(signature_scheme).await.unwrap());

        // Request X509 key packages
        let x509_key_packages = session
            .transaction
        ...
        assert!(x509_key_packages.iter().all(
            |kp| MlsCredentialType::X509 == MlsCredentialType::from(kp.leaf_node().credential().credential_type())
        ));
    }
