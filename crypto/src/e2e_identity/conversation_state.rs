...
impl CentralContext {
    /// See [MlsCentral::e2ei_conversation_state].
...
impl MlsCentral {
    ...
    #[cfg_attr(not(test), tracing::instrument(err, skip(self), fields(id = base64::Engine::encode(&base64::prelude::BASE64_STANDARD, id))))]
    pub async fn e2ei_conversation_state(&self, id: &ConversationId) -> CryptoResult<E2eiConversationState> {
        let conversation = self.get_conversation(id, &self.mls_backend.keystore()).await?;
        let Some(conversation) = conversation else {
            return Err(crate::CryptoError::ConversationNotFound(id.clone()));
        };
        conversation
            .e2ei_conversation_state(&self.mls_backend.new_transaction())
            .await
    }
    ...
    pub async fn e2ei_verify_group_state(
        ...
        let is_sender = true; // verify the ratchet tree as sender to turn on hardened verification
        let Ok(rt) = group_info.take_ratchet_tree(self.mls_backend.deref(), is_sender).await else {
            return Ok(E2eiConversationState::NotVerified);
        };
