...
export class CoreCryptoContext extends CoreCryptoContextFfi {
     ...
     * @returns The new {@link CoreCryptoFfiTypes.E2eiEnrollment} enrollment instance to use with {@link CoreCryptoContext.e2eiMlsInitializeOnly}
     */
    async e2eiNewEnrollment(
        ...
        team?: string,
        asyncOpts_?: { signal: AbortSignal }
    ): Promise<E2eiEnrollmentInterface> {
        return await super.e2eiNewEnrollment(
            clientId,
            displayName,
            handle,
            expirySec,
    ...
    async findCredentials(
        ...
        earliestValidity?: bigint,
        asyncOpts_?: { signal: AbortSignal }
    ): Promise<Array<CredentialRefInterface>> {
        return await super.findCredentials(
            clientId,
            publicKey,
            ciphersuite,
    ...
    async createConversation(
        conversationId: ConversationIdInterface,
        credentialRef: CredentialRefInterface,
        externalSender?: ExternalSenderKeyInterface,
        asyncOpts_?: { signal: AbortSignal }
    ): Promise<void> {
