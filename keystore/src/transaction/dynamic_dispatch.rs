...
use crate::{
    ...
    entities::{
        ...
        KeyPackageData, MlsBufferedCommit, MlsCredential, MlsEncryptionKeyPair, MlsEpochEncryptionKeyPair,
...
impl EntityId {
    fn as_id(&self) -> StringEntityId<'_> {
        match self {
            ...
            EntityId::KeyPackageData(vec) => vec.as_slice().into(),
            EntityId::PskBundle(vec) => vec.as_slice().into(),
            EntityId::Psk(vec) => vec.as_slice().into(),
    ...
    pub(crate) fn from_collection_name(entity_id: &'static str, id: &[u8]) -> CryptoKeystoreResult<Self> {
        match entity_id {
            ...
            KeyPackageData::COLLECTION_NAME => Ok(Self::KeyPackageData(id.into())),
            MlsPskBundle::COLLECTION_NAME => Ok(Self::PskBundle(id.into())),
            Psk::COLLECTION_NAME => Ok(Self::Psk(id.into())),
    ...
    pub(crate) fn collection_name(&self) -> &'static str {
        match self {
            ...
            EntityId::KeyPackageData(_) => KeyPackageData::COLLECTION_NAME,
            EntityId::PskBundle(_) => MlsPskBundle::COLLECTION_NAME,
            EntityId::Psk(_) => Psk::COLLECTION_NAME,
...
pub async fn execute_save(tx: &TransactionWrapper<'_>, entity: &Entity) -> CryptoKeystoreResult<()> {
    match entity {
        ...
        Entity::KeyPackageData(data) => data.save(tx).await,
        Entity::PskBundle(mls_psk_bundle) => mls_psk_bundle.save(tx).await,
        Entity::Psk(psk) => psk.save(tx).await,
...
pub async fn execute_delete(tx: &TransactionWrapper<'_>, entity_id: &EntityId) -> CryptoKeystoreResult<()> {
    match entity_id {
        ...
        id @ EntityId::KeyPackageData(_) => MlsKeyPackage::delete(tx, id.as_id()).await,
        id @ EntityId::PskBundle(_) => MlsPskBundle::delete(tx, id.as_id()).await,
        id @ EntityId::Psk(_) => Psk::delete(tx, id.as_id()).await,
