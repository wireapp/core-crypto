...
mod tests {
    ...
    async fn join_by_external_commit_should_succeed(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let (external_commit, mut pending_conversation) = conversation.unmerged_external_join_guarded(&bob).await;

            // Alice acks the request and adds the new member
            assert_eq!(external_commit.conversation().member_count().await, 1);
            let (external_commit, result) = external_commit.notify_member_failible(&alice).await;
    ...
    async fn join_by_external_commit_should_be_retriable(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            // Bob tries to join Alice's group
            let (commit_guard, _pending_conversation) = conversation.unmerged_external_join_guarded(&bob).await;
            ...
            // Retrying
            let conversation = commit_guard.finish().external_join(&bob).await;
            assert_eq!(conversation.member_count().await, 2);
            assert!(conversation.is_functional_with([&alice, &bob]).await);
        })
        .await
    ...
    async fn should_fail_when_bad_epoch(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let conversation = commit_guard.finish().update().await;
    ...
    async fn existing_clients_can_join(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let conversation = conversation.external_join(&alice).await;
            assert!(conversation.is_functional_with([&alice, &bob]).await);
        })
        .await
    }
    ...
    async fn should_return_valid_group_info(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            // Bob tries to join Alice's group
            let conversation = conversation.external_join(&bob).await;
            assert_eq!(conversation.member_count().await, 2);
            assert!(conversation.is_functional_with([&alice, &bob]).await);
            ...
            let conversation = conversation.external_join_via_group_info(&charlie, bob_gi).await;
    ...
    async fn clear_pending_group_should_succeed(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            // Bob tries to join Alice's group
            let (_, mut pending_conversation) = conversation.unmerged_external_join_guarded(&bob).await;
    ...
    async fn new_with_inflight_join_should_fail_when_already_exists(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            conversation.external_join_guarded(&bob).await;
            // erroneous call
    ...
    async fn new_with_inflight_welcome_should_fail_when_already_exists(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let id = conversation.id().clone();
                ...
                commit_guard.finish().invite_guarded([&bob]).await;
    ...
    async fn should_fail_when_invalid_group_info(case: TestContext) {
        ...
        let conversation = case.create_conversation([&alice]).await.invite([&bob]).await;
