...
mod tests {
    ...
    pub async fn create_many_people_conversation(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            let conversation = conversation.invite(bob_and_friends).await;

            assert_eq!(conversation.member_count().await, 1 + GROUP_SAMPLE_SIZE);
            assert!(conversation.is_functional_with(&alice_and_friends).await);
        })
        .await;
    }
    ...
    mod wire_identity_getters {
        ...
        async fn should_exchange_messages_cross_signed() {
            ...
            Box::pin(async move {
                ...
                assert!(conversation.is_functional_with(sessions).await);
            })
            .await;
        }
    }
