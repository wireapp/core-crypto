...
use crate::{
    ...
    unified_connection::idb_migration::legacy::traits::{Decryptable, Decrypting, Entity},
};
...
impl WasmEncryptedStorage {
    ...
    pub(crate) fn new_with_pre_v4_key(key: &str, storage: WasmStorageWrapper) -> Self {
    ...
    pub(crate) fn wrapper(&self) -> &WasmStorageWrapper {
        &self.storage
    }

    pub(crate) fn close(self) -> CryptoKeystoreResult<()> {
        match &self.storage {
            WasmStorageWrapper::Persistent(idb) => {
                idb.close();
            }
    ...
    pub(crate) async fn get<'a, E>(&self, key: impl AsRef<[u8]>) -> CryptoKeystoreResult<Option<E>>
    where
    ...
    pub(crate) async fn get_all<'a, E>(&self) -> CryptoKeystoreResult<Vec<E>>
    where
        E: Entity<ConnectionType = WasmConnection> + Decryptable<'a>,
        <E as Decryptable<'a>>::DecryptableFrom: DeserializeOwned,
    {
