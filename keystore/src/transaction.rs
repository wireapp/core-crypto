...
impl KeystoreTransaction {
    pub(crate) async fn new(db: Connection) -> CryptoKeystoreResult<Self> {
        Ok(Self {
            db,
     ...
     ($keystore_transaction:expr, [ $( ($records:ident, $entity:ty) ),*]) => {
        ...
        cfg_if::cfg_if! {
            if #[cfg(target_family = "wasm")] {
                ...
                let tables_str = tables.iter().map(|t| t.as_ref()).collect::<Vec<&str>>().join(", ");
                web_sys::console::log_1(&format!("Creating transaction for tables: {}", tables_str).into());
