...
use crate::{
    ...
    traits::{
        BorrowPrimaryKey, KeyType, UnifiedEntity, UnifiedEntityDatabaseMutation, UnifiedEntityDeleteBorrowed,
...
impl KeystoreTransaction {
    ...
    ///
    /// - Adjust the entity by calling its [`pre_save()`][UnifiedEntity::pre_save] method.
    ...
    pub(crate) async fn save<E>(&self, mut entity: E) -> CryptoKeystoreResult<E::AutoGeneratedFields>
    where
    ...
    pub(crate) async fn remove<E>(&self, id: &E::PrimaryKey) -> CryptoKeystoreResult<()>
    where
        E: UnifiedEntity + UnifiedEntityDatabaseMutation,
    {
    ...
    /// Note that this doesn't return whether or not anything was actually removed because
    /// that won't happen until the transaction is committed.
    pub(crate) async fn remove_borrowed<E>(&self, id: &E::BorrowedPrimaryKey) -> CryptoKeystoreResult<()>
    where
        E: UnifiedEntityDeleteBorrowed + BorrowPrimaryKey,
    {
    ...
    where
        E: 'static + UnifiedEntity + Send + Sync,
    {
    ...
    where
        E: 'static + UnifiedEntity + BorrowPrimaryKey + Send + Sync,
    {
        let entity_id = EntityId::from_borrowed_primary_key::<E>(id)?;
        self.get_by_entity_id(&entity_id).await
    }
    ...
    where
        E: 'static + UnifiedEntity + UnifiedSearchableEntity<SearchKey> + Send + Sync,
        SearchKey: KeyType,
    {
        let cache_guard = self.cache.read().await;
        cache_guard
    ...
    where
        E: 'static + Clone + UnifiedEntity + Send + Sync,
    {
    ...
    where
        E: 'static + Clone + UnifiedEntity + UnifiedSearchableEntity<SearchKey> + Send + Sync,
        SearchKey: KeyType,
    {
