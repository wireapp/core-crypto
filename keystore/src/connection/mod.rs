...
use crate::{
    ...
    traits::{
        BorrowPrimaryKey, CryptoTransaction, Entity, EntityDatabaseMutation, EntityDeleteBorrowed, EntityGetBorrowed,
        FetchFromDatabase, KeyType, PkiTransaction, SearchableEntity,
    },
...
impl Database {
    ...
    async fn new_transaction<const SLOT: usize>(&self) -> CryptoKeystoreResult<()> {
    ...
    async fn commit_transaction<const SLOT: usize>(&self) -> CryptoKeystoreResult<()> {
        let mut transaction_guard = self.transactions[SLOT].lock().await;
    ...
    async fn rollback_transaction<const SLOT: usize>(&self) -> CryptoKeystoreResult<()> {
        let mut transaction_guard = self.transactions[SLOT].lock().await;
    ...
    async fn save<'a, E, const SLOT: usize>(&self, entity: E) -> CryptoKeystoreResult<E::AutoGeneratedFields>
    where
    ...
    async fn remove<'a, E, const SLOT: usize>(&self, id: &E::PrimaryKey) -> CryptoKeystoreResult<()>
    where
    ...
    async fn remove_borrowed<'a, E, const SLOT: usize>(&self, id: &E::BorrowedPrimaryKey) -> CryptoKeystoreResult<()>
    where
...
#[cfg_attr(target_os = "unknown", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_os = "unknown"), async_trait::async_trait)]
impl CryptoTransaction for Database {
    const SLOT: usize = 0;

    async fn new_transaction(&self) -> CryptoKeystoreResult<()> {
        self.new_transaction::<{ <Self as CryptoTransaction>::SLOT }>().await
    }

    async fn commit_transaction(&self) -> CryptoKeystoreResult<()> {
        self.commit_transaction::<{ <Self as CryptoTransaction>::SLOT }>().await
    }

    async fn rollback_transaction(&self) -> CryptoKeystoreResult<()> {
        self.rollback_transaction::<{ <Self as CryptoTransaction>::SLOT }>()
            .await
    }
    ...
    where
        E: Entity + EntityDatabaseMutation<'a> + Send + Sync,
    {
        self.save::<E, { <Self as CryptoTransaction>::SLOT }>(entity).await
    }
    ...
    where
        E: Entity + EntityDatabaseMutation<'a>,
    {
        self.remove::<E, { <Self as CryptoTransaction>::SLOT }>(id).await
    }
    ...
    where
        E: Entity + EntityDatabaseMutation<'a> + BorrowPrimaryKey + EntityDeleteBorrowed<'a>,
    {
        self.remove_borrowed::<E, { <Self as CryptoTransaction>::SLOT }>(id)
            .await
    }
    ...
    async fn child_groups(&self, entity: PersistedMlsGroup) -> CryptoKeystoreResult<Vec<PersistedMlsGroup>> {
        ...
        let transaction_guard = self.transactions[<Self as CryptoTransaction>::SLOT].lock().await;
        let Some(transaction) = transaction_guard.as_ref() else {
...
#[cfg_attr(target_os = "unknown", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_os = "unknown"), async_trait::async_trait)]
impl PkiTransaction for Database {
    const SLOT: usize = 1;

    async fn new_transaction(&self) -> CryptoKeystoreResult<()> {
        self.new_transaction::<{ <Self as PkiTransaction>::SLOT }>().await
    }

    async fn commit_transaction(&self) -> CryptoKeystoreResult<()> {
        self.commit_transaction::<{ <Self as PkiTransaction>::SLOT }>().await
    }

    async fn rollback_transaction(&self) -> CryptoKeystoreResult<()> {
        self.rollback_transaction::<{ <Self as PkiTransaction>::SLOT }>().await
    }
    ...
    where
        E: Entity + EntityDatabaseMutation<'a> + Send + Sync,
    {
        self.save::<E, { <Self as PkiTransaction>::SLOT }>(entity).await
    }
    ...
    where
        E: Entity + EntityDatabaseMutation<'a>,
    {
        self.remove::<E, { <Self as PkiTransaction>::SLOT }>(id).await
    }
    ...
    where
        E: Entity + EntityDatabaseMutation<'a> + BorrowPrimaryKey + EntityDeleteBorrowed<'a>,
    {
        self.remove_borrowed::<E, { <Self as PkiTransaction>::SLOT }>(id).await
    }
}
