...
impl CoreCrypto<'_> {
    ...
    pub fn mls_generate_keypairs(&self, ciphersuites: Vec<CiphersuiteName>) -> CryptoResult<Vec<ClientId>> {
        let ciphersuites: Vec<MlsCiphersuite> = ciphersuites.into_iter().map(Into::into).collect();
        future::block_on(
            self.executor.lock().map_err(|_| CryptoError::LockPoisonError)?.run(
                self.central
                    .lock()
        ...
        Ok(future::block_on(
            self.executor.lock().map_err(|_| CryptoError::LockPoisonError)?.run(
                self.central
                    .lock()
                    .map_err(|_| CryptoError::LockPoisonError)?
                    .new_external_remove_proposal(conversation_id, epoch.into(), leaf_index.into(), cs, ct),
            ),
        )?
        .to_bytes()
        .map_err(MlsError::from)?)
