...
impl WasmEncryptedStorage {
    ...
    pub fn idb_version(&self) -> CryptoKeystoreResult<u32> {
        match self.storage {
            WasmStorageWrapper::Persistent(ref idb) => idb.version().map_err(Into::into),
            WasmStorageWrapper::InMemory(_) => Err(CryptoKeystoreError::IncorrectApiUsage(
                "Called idb_version() on in-memory wasm storage.",
            )),
        }
    }
