...
impl ConversationMut {
    ...
    pub(crate) async fn extract_confirmation_tag_from_own_commit<'a>(
        ...
        let MlsMessageInBody::PublicMessage(msg) = own_commit.body_as_ref() else {
            panic!(
    ...
    fn eq_pending_commit(group: &MlsGroup, commit_ct: &ConfirmationTag) -> bool {
        group
            .pending_commit()
            .is_some_and(|pending_commit| pending_commit.get_confirmation_tag() == commit_ct)
    }
