...
";
    ...
    struct MockPkiEnvironmentHooks;

    #[async_trait]
    impl PkiEnvironmentHooks for MockPkiEnvironmentHooks {
        async fn http_request(
            &self,
            _method: HttpMethod,
            _url: String,
    ...
    async fn can_add_trust_anchor() {
        let database = Arc::new(
            Database::in_memory(Arc::new(DatabaseKey::generate().into()))
                .await
                .unwrap(),
        );
        let pki_env = PkiEnvironment::new(Arc::new(MockPkiEnvironmentHooks), database)
            .await
            .unwrap();

        let result = pki_env.add_trust_anchor(EXAMPLE_CERT_PEM).await;
        dbg!(&result);
        assert!(result.is_ok());
    }
    ...
    async fn can_add_intermediate_cert() {
        ...
        let result = pki_env.add_intermediate_cert(EXAMPLE_CERT_PEM).await;
        dbg!(&result);
        assert!(result.is_ok());
    }
}
