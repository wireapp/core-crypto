...
/**
 * Data shape for the returned MLS proposal and commit message tuple upon leaving a conversation
 */
export interface ConversationLeaveMessages {
    /**
     * MLS Proposal of removal of the current client.
     * This needs to be fanned out **after** the `other_clients_removal_commit`
     *
     * @readonly
     */
    self_removal_proposal: Uint8Array;
    /**
     * MLS Commit that removes the other clients of the user
     * This **needs** to be fanned out first to the MLS Distribution Service
     *
     * @readonly
     */
...
export class CoreCrypto {
    ...
    /**
     * Leaves a conversation, along with the provided other Client IDs
     *
     * @param conversationId - The ID of the conversation
     * @param otherClients - Array of Client IDs to remove as well. There are a few internal checks to ensure the provided clients are also owned by the same user, but there's also no guarantee this is accurate
     *
     * @returns An object containing the TLS-serialized commits and proposals that result from this operation
     */
