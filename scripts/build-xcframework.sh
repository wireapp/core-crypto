#!/usr/bin/env bash
# Build WireCoreCryptoUniffi.xcframework and WireCoreCrypto.xcframework,
# zip them, and clean up the intermediate archives.
#
# Changes into `crypto-ffi/bindings/swift` for all xcodebuild calls.

set -euo pipefail

# move to the location of this file, then the git top level
# to ensure we're at the repo root
cd "$(dirname "$0")"
