...
mod tests {
    ...
    pub(crate) async fn can_run_migrations() {
        ...
        Database::migrate_db_key_type_to_bytes(name, "test1234", &TEST_ENCRYPTION_KEY)
            .await
            .unwrap();

        let mut conn = Database::migration_connection(test_builder(DB_VERSION_4), &TEST_ENCRYPTION_KEY)
            .await
            .expect("DB_VERSION_4");
        ...
        let migrated_db = Database::open(
            legacy::connection::ConnectionType::Persistent(name),
            &TEST_ENCRYPTION_KEY,
        )
        .await
    ...
    pub(crate) async fn v9_schema_allows_multiple_creds_per_session() {
        ...
        let builder = v09::get_builder(name);
        let mut conn = Database::migration_connection(builder, &TEST_ENCRYPTION_KEY)
            .await
            .expect("DB_VERSION_9");
        ...
        let migrated_db = Database::open(ConnectionType::Persistent(name), &TEST_ENCRYPTION_KEY)
            .await
            .expect("completing migrations");
