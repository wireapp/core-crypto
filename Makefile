...
JS_GEN := $(GEN_DIR)/core_crypto_ffi.js
WASM_GEN := $(GEN_DIR)/wasm-bindgen/index_bg.wasm
...
.PHONY: ts-test
ts-test: $(DTS_OUT) ## Run TypeScript wrapper tests via wdio and bun. Optionally pass TEST=<test> to filter by test name.
	@cd $(JS_DIR) && \
	if [ -n "$(TEST)" ]; then \
