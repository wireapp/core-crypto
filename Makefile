...
target/release/libcore_crypto_ffi.$(EXT) target/release/uniffi-bindgen: FORCE
	cargo build --release \
		--locked \
		--features uniffi/cli \
...
wasm-build.stamp: FORCE
	...
	$(WASM_PACK) build \
	  --locked \
	  --no-pack \
	  --out-dir crypto-ffi/bindings/js/src/autogenerated \
...
TS_SRCS   := $(shell find $(JS_SRC_DIR) \( -path $(JS_SRC_DIR)/autogenerated -o -path $(JS_SRC_DIR)/corecrypto.d.ts -prune \) -o \( -type f -name '*.ts' \) -print)
