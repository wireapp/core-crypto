...
impl TransactionContext {
    ...
    pub async fn mls_provider(&self) -> Result<MlsCryptoProvider> {
        match &*self.inner.read().await {
            TransactionContextInner::Valid { mls_client, .. } => {
                ...
                return Err(mls::session::Error::MlsNotInitialized)
                    .map_err(RecursiveError::mls_client(
                        "Getting mls session from transaction context",
                    ))
                    .map_err(Into::into);
    ...
    pub async fn init(
        ...
        let mut credential_refs =
            CredentialRef::find(database, CredentialFindFilters::builder().client_id(&client_id).build())
                .await
    ...
    pub async fn mls_init(
        ...
        let (client_id, identities) = self.init(identifier, ciphersuites, &database).await?;
