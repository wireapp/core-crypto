...
fn alter_configuration(host_volume: &Path, ca_cfg: &CaCfg) {
...
pub(crate) async fn start_acme_server(ca_cfg: &CaCfg) -> AcmeServer {
    ...
    let image = GenericImage::new("smallstep/step-ca", "0.29.0")
    ...
    // Note that `step` streams the output we're interested in to stderr.
    run_command_with_ready_conditions(
        &node,
        &format!(
        ...
        CmdWaitFor::message_on_stderr("Your PKI is ready to go."),
    )
    .await;
    ...
    run_command(
        &node,
        "bash -c 'step-ca --password-file password >/tmp/step-ca.log 2>&1 &'",
    )
    .await;
