...
use crate::{
    ...
    unified_connection::idb_migration::legacy::connection::{
...
impl WasmConnection {
    pub(crate) fn storage(&self) -> &WasmEncryptedStorage {
        &self.conn
    }

    #[cfg(test)]
    pub(crate) fn storage_mut(&mut self) -> &mut WasmEncryptedStorage {
        &mut self.conn
    }

    // for compatibility with the uniffi version
    pub(crate) async fn conn(&self) -> TransactionCreator<'_> {
...
impl<'a> DatabaseConnection<'a> for WasmConnection {
    ...
    async fn wipe(self) -> CryptoKeystoreResult<()> {
        if self.conn.is_persistent() {
            ...
            factory
                .delete(&self._name.expect("name is always set for a persistent connection"))?
                .await?;
        }
...
pub(crate) struct TransactionCreator<'a> {
