...
use openmls_traits::{OpenMlsCryptoProvider as _, random::OpenMlsRand};
use sha2::{Digest, Sha256};

use super::fixtures::*;
use crate::mls_provider::{CryptoProvider, EntropySeed};

fn test_randomness(backend: &mut CryptoProvider, entropy: Option<EntropySeed>) {
...
#[apply(use_provider)]
async fn can_generate_sufficient_randomness_ext_entropy(backend: CryptoProvider) {
    let mut backend = backend.await;
    test_randomness(&mut backend, Some(entropy()));
}

#[apply(use_provider)]
async fn can_generate_sufficient_randomness_sys_entropy(backend: CryptoProvider) {
    let mut backend = backend.await;
...
#[apply(use_provider)]
async fn can_be_externally_seeded_ietf_vectors_1_2(backend: CryptoProvider) {
...
#[apply(use_provider)]
async fn can_be_externally_seeded_ietf_vector_3(backend: CryptoProvider) {
    let backend = backend.await;
    // Test vector 3 from
    // https://tools.ietf.org/html/draft-nir-cfrg-chacha20-poly1305-04
    let seed = [
...
#[apply(use_provider)]
async fn can_be_externally_seeded_ietf_vector_4(backend: CryptoProvider) {
    let backend = backend.await;
    // Test vector 4 from
    // https://tools.ietf.org/html/draft-nir-cfrg-chacha20-poly1305-04
    let seed = [
...
#[apply(use_provider)]
async fn can_be_externally_seeded_ietf_vector_5(backend: CryptoProvider) {
    let backend = backend.await;
    // Test vector 5 from
    // https://tools.ietf.org/html/draft-nir-cfrg-chacha20-poly1305-04
    let seed = [0u8; 32];
