            ...
            run_test_with_client_ids(
                ...
                move |[mut alice_central, mut bob_central]| {
                    Box::pin(async move {
                        ...
                        alice_central.can_talk_to(&id, &mut bob_central).await.unwrap();
                    })
                },
            )
            .await
            ...
            run_test_with_client_ids(
                ...
                move |[mut alice_central, mut bob_central]| {
                    Box::pin(async move {
                        ...
                        assert!(alice_central.can_talk_to(&id, &mut bob_central).await.is_err());
                    })
                },
            )
            .await;
            ...
            run_test_with_client_ids(
                ...
                move |[mut alice_central, mut bob_central, mut charlie_central]| {
                    Box::pin(async move {
                        ...
                        alice_central.can_talk_to(&id, &mut bob_central).await.unwrap();
                        bob_central.can_talk_to(&id, &mut charlie_central).await.unwrap();
                        charlie_central.can_talk_to(&id, &mut alice_central).await.unwrap();
                    })
                },
            )
            .await;
