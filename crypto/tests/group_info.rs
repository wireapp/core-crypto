#[cfg(test)]
mod tests {
    use core_crypto::{prelude::{MlsConversationConfiguration, ConversationMember}, MlsCentral, MlsCentralConfiguration};
    ...
    fn group_info_size() {
        ...
        let clients = (0..=CLIENT_LIMIT).map(|_| {
            generate_client()
        ...
        for client in clients.iter() {
            ...
            let members_to_add = ConversationMember::new(client.client_id().unwrap().into(), kp[0].key_package().clone());
            root_client.add_members_to_conversation(&cid_bytes, &mut [members_to_add]).unwrap();
            let state = root_client.export_public_group_state(&cid_bytes).unwrap();
            lengths.push(state.len());
        }
