...
pub(super) enum HistoryClientUpdateResult {
...
impl ConversationGuard {
    ...
    pub(super) async fn update_history_client(&mut self) -> Result<HistoryClientUpdateResult> {
        if !self.is_history_sharing_enabled().await {
            return Ok(HistoryClientUpdateResult::NoUpdateNeeded);
        }
        ...
        let (existing_history_clients, other_clients): (Vec<_>, Vec<_>) =
        ...
        Ok(HistoryClientUpdateResult::CommitSentAndMerged)
    }
}
    ...
    async fn can_remove_two_and_add_one_member_in_commit(case: TestContext) {
        ...
        Box::pin(async move {
            ...
            assert_eq!(
                ...
                REMAINING_MEMBERS_COUNT + REMOVED_MEMBERS_COUNT
            );
