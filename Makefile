...
$(WASM_GEN): $(wasm-build-deps)
	...
	wasm-bindgen --target web \
	  ...
	  --out-name index \
	  --out-dir src/browser/autogenerated/wasm-bindgen \
...
$(BROWSER_OUT): $(ts-browser-deps)
	...
	cp src/browser/autogenerated/wasm-bindgen/index_bg.wasm out/browser/autogenerated/wasm-bindgen/ && \
	cp src/browser/autogenerated/wasm-bindgen/index_bg.wasm.d.ts out/browser/autogenerated/wasm-bindgen/ && \
	bun x dts-bundle-generator \
	  --project tsconfig.browser.json \
	  -o out/browser/corecrypto.d.ts \
	  --no-check \
	  --export-referenced-types false \
	  src/browser/CoreCrypto.ts
	@touch $@
...
$(TS_NATIVE_OUT): $(ts-native-deps)
	...
	bun build src/native/CoreCrypto.ts \
...
$(STAMPS)/ts-browser-test: $(ts-browser-test-deps)
	...
	if [ -n "$(TEST)" ]; then \
		...
		bun --conditions=cc-browser test --filter "$(TEST)" ./test/bun/wasm; \
	else \
		bun x wdio run ./wdio.test.conf.ts; \
		bun --conditions=cc-browser test ./test/bun/wasm; \
	fi
	$(TOUCH_STAMP)
...
$(STAMPS)/ts-native-test: $(ts-native-test-deps)
	...
	cd $(JS_DIR) && \
	if [ -n "$(TEST)" ]; then \
		bun --conditions=cc-native test --filter "$(TEST)" ./test/bun/napi; \
	else \
		bun --conditions=cc-native test ./test/bun/napi; \
	fi
	$(TOUCH_STAMP)
