use core_crypto_keystore::{
    entities::{
        StoredEpochEncryptionKeypair, TargetedMessageRxCounter, TargetedMessageRxCounterPk, TntSecret, TntSecretPk,
    },
...
use super::{
    Error, HpkeContextData, PskId, TargetedMessageContext, derive_targeted_message_psk, extract_hpke_context_data,
};
use crate::{
    CipherSuite, CryptoProvider, DecryptedMessage, KeystoreError, OpenMlsError, RecursiveError,
    mls::{
        conversation::{
            ConversationMut, MlsGroupState, Result, TargetedMessagePolicy,
...
impl ConversationMut {
    pub(in crate::mls::conversation::mutable::tnt) async fn decrypt_targeted(
        ...
        let existing_counter = database
            .get::<TargetedMessageRxCounter>(&counter_pk)
            .await
        ...
        let plaintext = VLBytes::tls_deserialize(&mut padded_plaintext.as_slice())
            .map_err(Error::tls_deserialize("TargetedMessageContent"))?
            .into();
    ...
    async fn load_hpke_decryption_data(
        ...
        let group_context = GroupContext::tls_deserialize(&mut secret.group_context.as_slice())
            .map_err(Error::tls_deserialize("TntSecret GroupContext"))?;
        ...
        let psk_id = PskId::new(mls_group.group_id().clone(), message.epoch)
            .tls_serialize_detached()
            .map_err(Error::tls_serialize("PskId"))?;
    ...
    pub(in crate::mls::conversation) async fn create_tnt_secret(&self, mls_group: &MlsGroup) -> Result<TntSecret> {
        ...
        Ok(TntSecret {
            ...
            group_context: mls_group
                ...
                .tls_serialize_detached()
                .map_err(Error::tls_serialize("TntSecret GroupContext"))?,
