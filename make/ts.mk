...
$(BUN_LOCK) $(NODE_MODULES) &: $(PACKAGE_JSONS) $(BUNFIG)
...
ts-browser-deps := $(TS_BROWSER_SRCS) $(WASM_GEN) $(BUN_LOCK) $(NODE_MODULES)
$(BROWSER_OUT) &: $(ts-browser-deps)
	...
	mkdir -p $(BROWSER_OUT_DIR)/autogenerated/wasm-bindgen && \
	cp packages/browser/src/autogenerated/wasm-bindgen/index_bg.wasm $(BROWSER_OUT_DIR)/autogenerated/wasm-bindgen/ && \
	cp packages/browser/src/autogenerated/wasm-bindgen/index_bg.wasm.d.ts $(BROWSER_OUT_DIR)/autogenerated/wasm-bindgen/ && \
	bun x dts-bundle-generator \
	  --project packages/browser/tsconfig.json \
	  -o $(BROWSER_OUT_DIR)/corecrypto.d.ts \
	  --no-check \
...
$(TS_NATIVE_OUT) &: $(ts-native-deps)
	...
	bun build packages/native/src/CoreCrypto.ts \
	  ...
	  --external "@ubjs/core" \
	  --format esm \
	  --outdir $(TS_NATIVE_OUT_DIR) \
	  --entry-naming "corecrypto.[ext]" && \
	cp $(TS_NATIVE_FFI_LIB) $(TS_NATIVE_OUT_DIR)/libcore_crypto_ffi.$(LIBRARY_EXTENSION) && \
	bun x dts-bundle-generator \
	  --project packages/native/tsconfig.json \
	  -o $(TS_NATIVE_OUT_DIR)/corecrypto.d.ts \
	  --no-check \
	  --export-referenced-types false \
...
ts-native-darwin: ## Build the native TypeScript wrapper for aarch64-apple-darwin
	$(MAKE) TS_NATIVE_TARGET_TRIPLE=$(TS_NATIVE_DARWIN_TARGET) ts-native
...
ts-native-linux: ## Build the native TypeScript wrapper for x86_64-unknown-linux-gnu
	$(MAKE) TS_NATIVE_TARGET_TRIPLE=$(TS_NATIVE_LINUX_TARGET) ts-native
