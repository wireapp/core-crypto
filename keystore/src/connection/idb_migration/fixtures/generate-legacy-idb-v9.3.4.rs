...
use core_crypto::{
    DatabaseKey, MlsTransportData, MlsTransportResponse,
    prelude::*,
    transaction_context::{Error as TransactionError, TransactionContext},
};
...
async fn session(
    ...
    tx.mls_init(
        ClientIdentifier::Basic(ClientId::from(client_id.as_bytes())),
        vec![ciphersuite],
        Some(2),
    )
    .await
...
async fn dump_legacy_fixture() {
    ...
    let (_bob_cc, bob, bob_transport) = session(
